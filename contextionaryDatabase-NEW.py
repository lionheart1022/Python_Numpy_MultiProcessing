...
class Document(object):
    def __init__(self, doc_id, documentlocation, phraseMaxLength, contextname):
        ...
        self.phraseTable = []
        self.phraseCountInDocument=dict()
    ...
    def getPhraseCount(self):
            ...
            for length in range(1, self.phraseMaxLength + 1):
                for phrase in self.textProcessor.getPhraseCount()[length].keys():
                    ...
                    if not phrase_id:
                        cur.execute(
                            '''
                              INSERT INTO phrase ("phrase_text", "phrase_index", "phrase_length") 
                              VALUES (%s, %s, %s)
                            ''', (phrase, defaultIndex, length))
                    ...
                    """
                    NEW IDEA: Create a tuple key dictionary for the document, that will record the count of each
                    phrase in the document
                    """
                    # Update phrase count per document

                    self.phraseCountInDocument.update({(self.doc_id[0],phrase_id[0],length):self.textProcessor.getPhraseCount()[length][phrase]})
        ...
        return self.phraseCountInDocument
...
class Database(object):
        ...
        try:
            ...
            for c_list in completed_list:
                ...
                cur.execute('''insert into context 
                                ...
                                "context_picture","context_level","icindex","rcindex") 
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)''', (
                    context_id,
                    context_immediate_parent_id,
                    context_name, context_children_id, context_picture, context_level,ICIndex,RCIndex))
        finally:
            cur.close()
            con.close()
    ...
    def add_document(self, file_path):
        ...
        """
        and not name.startwith("_classifier_") ADD THIS LATER
        """
        ...
        try:
            if name.endswith(".txt") and (docpathcount[0] == 0):
                ...
                """
                NEW CODE. MERGE specific document phrase count dictionary with master document phrase count
                dictionary
                """
                #self.updateMasterPhraseCount(document.getPhraseCount())
                miniPhraseCount=document.getPhraseCount()
                print(miniPhraseCount)
                #print(miniPhraseCount)
                import csv
                
                
                with open('phraseCountPerDocument.csv', 'a',newline='') as csv_file:
                    writer = csv.writer(csv_file)
                    for (documentkey,phrasekey,length), value in miniPhraseCount.items():
                        writer.writerow([cont_id[0],phrasekey,length,documentkey,value])
